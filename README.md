...
and generates a ~7,000-page static site from it — running unattended on a home
...
(PlanIt) that has no bulk export, no usable search interface, and aggressively
rate-limits. Anyone wanting to know what is being built on their street has to
click through it by hand.

This project turns that into a queryable dataset and a set of indexable pages:
one per application, plus hubs per postcode outcode and per application type.
It is the excuse for the engineering, not the other way round — the interesting
parts are the pacing against an uncooperative API, the atomicity of the daily
rebuild, and the generation of correct date metadata at scale.
...
- **Ingest** is async (`aiohttp` + `aiosqlite`) and self-pacing: the API replies
  `Rate limits exceeded / try again in Ns`, which the pacer turns into a
  sustainable interval instead of blocking for the whole advertised window.
- **Generate** renders from the DB with Jinja2 — no DB access in templates, one
  pass per page type, a sitemap and JSON-LD emitted as a side effect of the same
  data.
- **Deploy** never swaps the served directory. A bind-mounted `output/` must keep
  its inode, so the new build is overlaid onto the existing tree and stale files
...
| `build_staging.py` + `deploy_staging.py` | Safe rebuild for the live site: builds into `output_staging/`, then overlays it onto `output/` without replacing the directory (a bind-mounted dir must keep its inode). Use these instead of `generate.py` when you don't want a mid-build crash to leave the served site empty. |
...
- `sitemap.xml` carries a `<lastmod>` on every URL. Application pages use the
  council record's own `last_changed` date (the page only changes when the
...
- Application `<title>`s are built by `build_title()`, which truncates on a word
  ...
  that chopped mid-word and shipped `...Scholes Leeds L — Full` on every page.
...
starting a new one — so a hung run can't stack up behind the next schedule.

## Paths

- Project: `/volume1/dev/leeds-planning-seo` (NAS) → `/workspace/dev/leeds-planning-seo`
